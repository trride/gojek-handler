...
async function getPrice (book = false) {
    ...
    const motorBikePriceResult = await gojek.getMotorBikePrice(calculatedDetail.start.origin_lat_long, calculatedDetail.end.destination_lat_long);
...
async function bookRide (calculatedDetail, motorBikePriceResult) {
    const bookingData = await gojek.booking(calculatedDetail.start, calculatedDetail.end, motorBikePriceResult.price.estimate_token, device_token, gcm_key);
...
async function getPlace () {
    let test = await gojek.getPlaceNameFromLatLong("-7.115222030783675,107.43785351514816");
    console.log(test);
}
...
async function getAcceptedCurrentBookingByOrderNo (orderNo) {
    let acceptedCurrentBookingDetail = await gojek.getAcceptedCurrentBookingByOrderNo(orderNo);
